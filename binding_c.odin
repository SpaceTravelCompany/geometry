...
@(private = "file")
_c_allocator: runtime.Allocator = {
	procedure = c_allocator_proc,
	data      = nil,
}

@(private = "file")
alloc_func: proc "c" (size: c.size_t) -> rawptr
@(private = "file")
calloc_func: proc "c" (nmemb, size: c.size_t) -> rawptr
@(private = "file")
realloc_func: proc "c" (old: rawptr, size: c.size_t) -> rawptr
@(private = "file")
...
@(export)
geometry_set_alloc_func :: proc "c" (func: proc "c" (size: c.size_t) -> rawptr) {
	alloc_func = func
}
@(export)
geometry_set_calloc_func :: proc "c" (func: proc "c" (nmemb, size: c.size_t) -> rawptr) {
	calloc_func = func
}
@(export)
geometry_set_realloc_func :: proc "c" (func: proc "c" (old: rawptr, size: c.size_t) -> rawptr) {
	realloc_func = func
}
@(export)
geometry_set_free_func :: proc "c" (func: proc "c" (data: rawptr)) {
	free_func = func
}
...
@(private = "file")
_geometry_shapes_compute_polygon_impl :: proc(
	input: ^Geometry_Shapes,
	out: ^Geometry_RawShape,
) -> Geometry_Error {
	...
	out.vertices = (Geometry_Slice)(
		runtime.Raw_Slice{data = raw_data(res.vertices), len = len(res.vertices)},
	)
	out.indices = (Geometry_Slice)(
		runtime.Raw_Slice{data = raw_data(res.indices), len = len(res.indices)},
	)
...
geometry_fixed_from_f64 :: proc "c" (v: f64) -> Geometry_Fixed {
	f := fixed_bcd.from_f64(DEF_FRAC_DIGITS, v)
	return transmute(Geometry_Fixed)(f.i)
}

@(export)
geometry_fixed_to_f64 :: proc "c" (v: Geometry_Fixed) -> f64 {
	return fixed_bcd.to_f64(fixed_bcd.BCD(DEF_FRAC_DIGITS){i = transmute(i128)(v)})
}
...
@(private = "file")
_geometry_shapes_compute_polygon_fixed_impl :: proc(
	input: ^Geometry_Shapes_Fixed,
	out: ^Geometry_RawShape_Fixed,
) -> Geometry_Error {
	if input == nil || out == nil do return .None
	...
	// Direct cast - Geometry_Slice has same layout as Odin slice, no copy
	poly := shapes_fixed(DEF_FRAC_DIGITS) {
		nodes = (^[]shape_node_fixed(DEF_FRAC_DIGITS))(&input.nodes)^,
	}

	res, err := shapes_compute_polygon_fixed(poly, _c_allocator)
	if err != nil {
		#partial switch e in err {
		case __shape_error:
			#partial switch e {
...
geometry_raw_shape_free_fixed :: proc "c" (shape: ^Geometry_RawShape_Fixed) {
	...
	raw := raw_shape_fixed(DEF_FRAC_DIGITS) {
		vertices = (^[]shape_vertex2d_fixed(DEF_FRAC_DIGITS))(&shape.vertices)^,
		indices  = (^[]u32)(&shape.indices)^,
	}
	raw_shape_fixed_free(raw, _c_allocator)
}
